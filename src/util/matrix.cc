 ...
 * Note:
 * - Only square matrix could be identity matrix. Set initMat as MATRIX_IDENTITY for a
 ...
 */
void Matrix::init(int rows, int cols, int initMat)
{
    ...
    for (i = 0; i < rows; i++) {
        ...
        for (j = 0; j < cols; j++) {
            cell[i][j] = 0;
            if (initMat == MATRIX_IDENTITY && rows == cols && i == j) cell[i][j] = 1;
        }
    }
}
 ...
 * non-square matrix have no effect and the matrix is still set to zero.
 */
Matrix::Matrix(int rows, int cols, int initMat)
{
